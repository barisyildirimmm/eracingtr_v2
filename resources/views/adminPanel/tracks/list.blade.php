    ...
    <div class="container-fluid">
        ...
        <div class="page-header-card">
            <h2 class="mb-2 fw-bold">
                <i class="fas fa-road me-2"></i>{{ __('common.track_list') }}
            </h2>
            <p class="mb-0 opacity-90">
                <i class="fas fa-info-circle me-2"></i>
                {{ __('common.track_list_desc') }}
            </p>
        </div>
        ...
        <div class="tracks-table-card">
            <div class="table-header">
                <h5 class="mb-0 fw-bold">
                    <i class="fas fa-list me-2"></i>{{ __('common.tracks_list') }}
                </h5>
                <button class="btn-add" data-bs-toggle="modal" data-bs-target="#createTrackModal">
                    ...
                    <table id="tracksTable" class="table table-striped table-bordered">
                        ...
                        @foreach ($tracks as $track)
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-edit action-btn" data-bs-toggle="modal" data-bs-target="#editTrackModal" onclick="editTrack({{ $track->id }}, '{{ $track->name }}')">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="createTrackModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>{{ __('common.add_new_track_modal') }}
                </h5>
            ...
            <div class="modal-body p-4">
                <form id="createTrackForm">
                    <div class="mb-3">
                        <label for="name" class="form-label">{{ __('common.track_name') }}</label>
                        <input type="text" class="form-control" id="name" name="name" required placeholder="{{ __('common.track_name_placeholder') }}">
                    </div>
                </form>
            </div>
            <div class="modal-footer border-top-0 p-4">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('common.cancel') }}</button>
                <button type="button" class="btn btn-primary" onclick="createTrack()">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="editTrackForm">
                    ...
                    <div class="mb-3">
                        <label for="editName" class="form-label">{{ __('common.track_name') }}</label>
                        <input type="text" class="form-control" id="editName" name="name" required placeholder="{{ __('common.track_name_placeholder') }}">
                    </div>
                </form>
            </div>
            <div class="modal-footer border-top-0 p-4">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __('common.cancel') }}</button>
                <button type="button" class="btn btn-primary" onclick="updateTrack()">
                    <i class="fas fa-save me-2"></i>{{ __('common.save') }}
                </button>
            </div>
        </div>
...
@section('js')
    ...
    <script>
        ...
        function updateTrack() {
            const id = $('#editId').val();
            $.ajax({
                url: '{{ route('admin.tracks.edit', '') }}/' + id,
                type: 'PUT',
                data: {
                    _token: '{{ csrf_token() }}',
                    name: $('#editName').val()
